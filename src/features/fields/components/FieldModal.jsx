...
export const FieldModal = ({ isOpen, onClose, field }) => {
  ...
  try {
    ...
    formData.append("pricePerHour", data.pricePerHour); // Ahora usamos el nombre correcto
    ...
    await saveField(formData, field?._id);
    
    showSuccess(field ? "Campo actualizado" : "Campo creado");
    reset();
    setPreview(null);
    onClose(); 
  } catch (error) {
